...
for idx = 1:1:10
    load(['./data/100408_REST1LR/recon_img/img' num2str(idx-1) '.mat'], 'recon_L', 'recon_R');
    % (120, 1, 192, 192) --permute & reshape & tranpose-->
    % (29696, 36864) * (36864,120) -> (29696, 120)
    corticalrecon_L = Left_inverse_transformation * double(reshape(permute(recon_L,[1,2,4,3]),batchsize, [])');
    % (29716, 36864) * (36864,120) -> (29716, 120)
...
cii = ft_read_cifti('./data/100408_rfMRI_REST1_LR_Atlas_MSMAll_hp2000_clean_preprocessed.dtseries.nii');
cii_test = ft_read_cifti('./data/rfMRI_REST2_LR_Atlas_MSMAll_hp2000_clean.dtseries.nii');
